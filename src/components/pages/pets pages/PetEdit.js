...
function PetEdit() {
    ...
    return(
        ...
        <div className={style.container}>  
                ...
                <input className={style.form_field} defaultValue={petInfo.name} onChange={(event)=>{setNewName(event.target.value)}}/>
        
                <label className={style.form_label}>Date of birth:</label>
                <input className={style.form_field} type="date" defaultValue={petInfo.dob} onChange={(event)=>{setNewDOB(event.target.value)}}/>

                <label className={style.form_label}>Age:</label>
                <input className={style.form_field} type="number" defaultValue={petInfo.age} onChange={(event)=>{setNewAge(event.target.value)}}/>

                <label className={style.form_label}>Type:</label>
                <select className={style.form_field} defaultValue={petInfo.type} onChange={(event)=>{setNewType(event.target.value)}}>
                ...
                <input className={style.form_field} defaultValue={petInfo.gender}  onChange={(event)=>{setNewGender(event.target.value)}}/>

                <label className={style.form_label}>Location:</label>
                <input className={style.form_field} defaultValue={petInfo.location} onChange={(event)=>{setNewLocation(event.target.value)}}/>
                ...
                <textarea className={style.form_textarea} defaultValue={petInfo.description} rows="10" onChange={(event)=>{setNewDescription(event.target.value)}}/>
                <NavLink to={`/pet/${params.type}`}>
                   <button className={style.upload_button} onClick={updatePet}> Update Profile </button>
                </NavLink>
