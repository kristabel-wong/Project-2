...
function User() {
  ...
  const getLikedUsers = async (uid) => {
    ...
    const q = query(collection(db, "users"), where("__name__", "in", favIdArr));
  ...
  const adoptee = async function (adoptor, petId) {
    const getUserData = async function (user) {
      const UserDoc = doc(db, "users", user);
      const DocSnap = await getDoc(UserDoc);
      return DocSnap.data();
    };
    const currentUserData = await getUserData(adoptor.value);
    // console.log("adoptor button clicked", adoptor.value, petId);
    ...
    const userDoc = doc(db, "users", adoptor.value);
  ...
  const editAdoptee = async function (adoptor, petId) {
    const getUserData = async function (user) {
      const UserDoc = doc(db, "users", user);
      const DocSnap = await getDoc(UserDoc);
      return DocSnap.data();
    ...
    const currentUserData = await getUserData(adoptor.value);
    const userDoc = doc(db, "users", adoptor.value);
    const index = currentUserData.adoptedPets.indexOf(adoptor.value);
    await updateDoc(userDoc, {
              ...
              <div>
                {params.id === auth.currentUser.uid ? (
                  <div className={style.edit}>
                    ...
                    <NavLink to={"/newpet"}>
                      <button className={style.button74}>New Pet</button>
                    </NavLink>
                  </div>
                ) : (
                  ""
                    ...
                    <div key={pet.id}>
                      ...
                      <div className={style.select}>
                        {!pet.adoptedBy ? (
                          ...
                          <div>
                            <h5>Adoptor: {pet.adoptedBy.label}</h5>
                            <div className={style.btnContainer}>
                              <button
                                className={style.button74}
                                onClick={() =>
                                  editAdoptee(pet.adoptedBy, pet.id)
                                }
                              >
                                Edit
                              </button>
          ...
          {userFavPets.length < 1 ? (
            ...
            <div>
              ...
              <div className={style.pets}>
                {userFavPets.map((pet) => {
                  return (
                    <div className={style.liked} key={pet.id}>
                      <NavLink to={`/pet/${pet.id}`} key={pet.id}>
                        <div key={pet.id} className={style.petTile}>
                          <div
                            className={style.image}
